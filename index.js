...
function getArtistByIndex(array, index) {
    ...
    return `Artist at index ${index} is ${name}`
  }
...
function get20s(year){
  ...
  for (let i = 0; i < artists.length; i++){
    // if(artists[i].years >= year && reviews[i].years <= year){
    //   twenties.push(reviews[i].name);
    // }//if
...
function removeArtist(array, indexNum) {
    /* code here */
    artists.splice(indexNum, 1); //remove 1 element at w.e index the flavor that was chosen in on
    return array;
...
function addArtist(id,name,years,genre,nationality,bio){
    ...
    /* Code here */
  // artists.push({id:20,name:'Shazeen Fabius',years:'1993-current',genre:'Web Dev',nationality:'Haitian',bio:'I love my dogs and popcorn.'})
  let addMe = artists.push({id,name,years,genre,nationality,bio});
  return addMe;
  }
  console.log(addArtist(20,'Shazeen Fabius','1993-current','Web Dev','Haitian','I love my dogs and popcorn.'));
  console.log(artists);
